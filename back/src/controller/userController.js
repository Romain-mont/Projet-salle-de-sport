...
const userController = {
    ...
    async store(req, res, next) {
        try {
            ...
            const first_name = sanitize(req.body.first_name, { allowedTags: [] }).trim();
            const last_name = sanitize(req.body.last_name, { allowedTags: [] }).trim();
            const email = sanitize(req.body.email, { allowedTags: [] }).trim();
            const password = sanitize(req.body.password, { allowedTags: [] }).trim();
            const confirmation = sanitize(req.body.confirmation, { allowedTags: [] }).trim();
            ...
            const createUserSchema = Joi.object({
                ...
                password: Joi.string().min(8).pattern(new RegExp("^(?=.*[A-Z])(?=.*\\d).{8,}$")).required(),
                confirmation: Joi.string().valid(Joi.ref("password")).required().messages({
                    "any.only": "Les mots de passe ne correspondent pas."
                })
            });

            const { error } = createUserSchema.validate({ first_name, last_name, email, password, confirmation });
    ...
    async update(req, res, next) {
        ...
        try {
            ...
            const updateUserSchema = Joi.object({
                ...
                password: Joi.string().min(8).pattern(new RegExp("^(?=.*[A-Z])(?=.*\\d).{8,}$")).optional()
            });

            const { error } = updateUserSchema.validate({ first_name, last_name, email, password });
