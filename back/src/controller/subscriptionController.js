...
const subscriptionController = {
	...
	async store(req, res, next) {
		try {
			...
			const price = Number.parseFloat(req.body.price); // Vérification avant sanitize()
	...
	async update(req, res, next) {
		...
		try {
			...
			const updateSubscriptionSchema = Joi.object({
				type: Joi.string()
					.valid("mensuel", "annuel", "sans engagement")
					.optional(),
		...
		try {
			...
			if (!subscription) {
				return res.status(404).json({
					message: `Abonnement avec l'ID ${subscriptionId} non trouvé.`,
				});
			}
			...
			if (user.subscription_id) {
				return res
					.status(409)
					.json({ message: "L'utilisateur a déjà un abonnement actif." });
			}
			...
			res.status(200).json({
				message:
					"Abonnement souscrit avec succès. L'utilisateur est maintenant abonné.",
				user,
				subscription,
			});
		} catch (error) {
	...
	async removeUserSubscription(req, res, next) {
		try {
			...
			if (!user.subscription_id) {
				return res
					.status(400)
					.json({ message: "L'utilisateur n'a pas d'abonnement actif." });
			}
